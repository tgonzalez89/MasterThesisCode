...
try:
    while True:
        ...
        model.add(Conv2D(6, (1,1), strides=(1,1), padding='valid', activation='relu'))
        model.add(Conv2D(6, (1,1), strides=(1,1), padding='valid', activation='relu'))
        ...
        for epoch in range(1000):
            ...
            if epoch >= 50 and history.history['mean_absolute_percentage_error'][0] >= 3:
                ...
                break
            if epoch >= 250 and history.history['mean_absolute_percentage_error'][0] >= 2:
                print("Not converging fast enough. Trying again...")
                break
            '''if history.history['mean_absolute_percentage_error'][0] < 1.0:
                print("Acceptable state reached. Breaking early...")
                break'''
        if history.history['mean_absolute_percentage_error'][0] < 2.0:
            break
