...
int main(int argc, const char* argv[]) {
    ...
    InferenceEngine::CNNNetwork network = core.ReadNetwork("nn_inversek2j-2D.xml", "nn_inversek2j-2D.bin");
    ...
    const int BLOCK_SIZE = input_data->getTensorDesc().getDims().at(2) * (input_data->getTensorDesc().getDims().at(3) / 2);

    std::cout << "NN input shape: " <<
        input_data->getTensorDesc().getDims().at(0) << ", " <<
        input_data->getTensorDesc().getDims().at(1) << ", " <<
        input_data->getTensorDesc().getDims().at(2) << ", " <<
        input_data->getTensorDesc().getDims().at(3) << std::endl;
    ...
    for (int i = 0; i < n * 4; i += 4 * BLOCK_SIZE) {
        if (InferenceEngine::OK == infer_request[ir_count]->Wait(InferenceEngine::IInferRequest::WaitMode::RESULT_READY)) {
            ...
            for (int block = 0; block < BLOCK_SIZE; block++) {
                res[iters[ir_count] + block*2 + 0] = output_buffer[block];
                res[iters[ir_count] + block*2 + 1] = output_buffer[BLOCK_SIZE+block];
            }
        }
    ...
    for (unsigned int ir = 0; ir < ir_size; ir++) {
        if (InferenceEngine::OK == infer_request[ir]->Wait(InferenceEngine::IInferRequest::WaitMode::RESULT_READY)) {
            ...
            float* output_buffer = output->buffer().as<InferenceEngine::PrecisionTrait<InferenceEngine::Precision::FP32>::value_type*>();
            for (int block = 0; block < BLOCK_SIZE; block++) {
                res[iters[ir] + block*2 + 0] = output_buffer[block];
                res[iters[ir] + block*2 + 1] = output_buffer[BLOCK_SIZE+block];
            }
        }
    }
