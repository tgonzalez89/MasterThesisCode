...
for f in `ls data/*.png`
do
    name=$(basename $f)
    name=${name%.*}
    ./sobel-orig.out $f results/$name-orig.png
    ./sobel-openvino-2D-async.out $f results/$name-ov.png CPU 4
    ../common/img_diff.py results/$name-orig.png results/$name-ov.png results/$name-diff.png
done
...
do
    ...
    name=${name%.*}
    ./sobel-orig.out $f results/$name-orig.png
    ./sobel-openvino-2D-async.out $f results/$name-ov.png MYRIAD 4
    ../common/img_diff.py results/$name-orig.png results/$name-ov.png results/$name-diff.png
done
