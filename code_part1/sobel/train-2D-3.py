...
def apply_sobel(img):
    ...
    retval, sobel = cv2.threshold(sobel, 0.7071, 0.7071, cv2.THRESH_TRUNC)
    return sobel
...
try:
    while True:
        ...
        model.add(ReLU(max_value=0.7071))
        optimizer = optimizers.SGD(learning_rate=0.01, momentum=0.9, nesterov=True)
        #optimizer = optimizers.RMSprop(learning_rate=0.5)
        model.compile(optimizer=optimizer, loss='mse', metrics=['mape'])
        model.summary()
        for epoch in range(1000):
            print(f"Epoch #{epoch}")
            history = model.fit(inputs, outputs, batch_size=16, epochs=1, validation_split=0.2)
            if history.history['loss'][0] >= 0.15:
                print("Starting weights/biases not good. Trying again...")
                break
            if epoch >= 10 and history.history['loss'][0] >= 0.008:
                print("Not converging fast enough. Trying again...")
                break
            if epoch >= 20 and history.history['loss'][0] >= 0.005:
                print("Not converging fast enough. Trying again...")
                break
            if epoch >= 50 and history.history['loss'][0] >= 0.004:
                print("Not converging fast enough. Trying again...")
                break
            if epoch >= 250 and history.history['loss'][0] >= 0.002:
                print("Not converging fast enough. Trying again...")
                break
            ...
            if history.history['loss'][0] < 0.0025:
                ...
                break'''
        if history.history['loss'][0] < 0.002:
            break
